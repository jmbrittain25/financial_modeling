...
def real_estate_factory(params: Dict[str, Any]) -> fs.Simulation:
    ...
    sim = fs.Simulation("RealEstateSim", start, end)
    ...
    sim.add_builder(fs.FixedValueGenerator(-down_payment - closing_fees, dt.timedelta(days=0), {'type': 'purchase'}))  # One-time at start
...
if __name__ == "__main__":
    ...
    dists = {
        ...
        'closing_fees': fs.TriangularDistribution(5000, 10000, 16000),  # 2-5% of $300k
        'seller_rate': fs.UniformDistribution(0.04, 0.06),  # Family deal 4-6%
        'seller_term_months': fs.UniformDistribution(60, 120),  # 5-10 years
        'kitchen_cost': fs.TriangularDistribution(25000, 40000, 75000),  # Renov costs
        'floors_cost': fs.TriangularDistribution(5000, 10000, 15000),
        'central_air_cost': fs.TriangularDistribution(5000, 7000, 10000),
    ...
    runner = fs.SimulationRunner(builder)
    sims = runner.run(100)
