...
def get_default_vg(vg_type: str) -> AnyValueGenerator:
    ...
    if vg_type == "VariableRateLoan":
        return VariableRateLoanValue(principal=250000.0, initial_rate=0.065, term_months=180, rate_key="mortgage_rate")
    if vg_type == "Dividend":
    ...
    if vg_type == "RateChange":
        return RateChangeValue(dist=NormalDistribution(mean=0.065, std=0.008), update_key="market_rate")
...
def render_value_generator_editor(
    ...
    # --- Type-specific forms ---
    if vg_type == "Fixed":
        val = float(getattr(initial, "value", 1000.0)) if (initial and getattr(initial, "type", None) == "Fixed") else 1000.0
        v = st.number_input("Fixed value each period (positive = inflow)", value=val, step=50.0, key=f"{key_prefix}_fixed_val")
        return FixedValue(value=float(v))

    if vg_type == "Growing":
        init = float(getattr(initial, "initial", 1000.0)) if (initial and getattr(initial, "type", None) == "Growing") else 1000.0
        g = float(getattr(initial, "growth_rate", 0.03)) if (initial and getattr(initial, "type", None) == "Growing") else 0.03
        c1, c2 = st.columns(2)
        iv = c1.number_input("Starting value", value=init, step=50.0, key=f"{key_prefix}_grow_init")
        gr = c2.number_input("Annual growth rate (e.g. 0.03 = 3%)", value=g, step=0.005, format="%.3f", key=f"{key_prefix}_grow_rate")
    ...
    if vg_type == "Distribution":
        ...
        if initial and getattr(initial, "type", None) == "Distribution" and hasattr(initial, "dist"):
    ...
    if vg_type == "VariableRateLoan":
        p = float(getattr(initial, "principal", 300000.0)) if (initial and getattr(initial, "type", None) == "VariableRateLoan") else 300000.0
        r = float(getattr(initial, "initial_rate", 0.065)) if (initial and getattr(initial, "type", None) == "VariableRateLoan") else 0.065
        t = int(getattr(initial, "term_months", 180)) if (initial and getattr(initial, "type", None) == "VariableRateLoan") else 180
        rk = getattr(initial, "rate_key", "mortgage_rate") if (initial and getattr(initial, "type", None) == "VariableRateLoan") else "mortgage_rate"
        c1, c2, c3 = st.columns(3)
        prin = c1.number_input("Principal ($)", value=p, step=5000.0, key=f"{key_prefix}_loan_prin")
        rate = c2.number_input("Initial rate (e.g. 0.065)", value=r, step=0.001, format="%.3f", key=f"{key_prefix}_loan_rate")
        term = c3.number_input("Term (months)", value=t, min_value=12, max_value=600, step=12, key=f"{key_prefix}_loan_term")
        rate_key = st.text_input("State key that will hold the current rate", value=rk, key=f"{key_prefix}_loan_rate_key")
        return VariableRateLoanValue(principal=float(prin), initial_rate=float(rate), term_months=int(term), rate_key=rate_key)

    if vg_type == "Dividend":
        y = float(getattr(initial, "annual_yield", 0.025)) if (initial and getattr(initial, "type", None) == "Dividend") else 0.025
        k = getattr(initial, "investment_value_key", "portfolio_value") if (initial and getattr(initial, "type", None) == "Dividend") else "portfolio_value"
        c1, c2 = st.columns(2)
        yld = c1.number_input("Annual yield (e.g. 0.028)", value=y, step=0.001, format="%.3f", key=f"{key_prefix}_div_yld")
    ...
    if vg_type == "InvestmentContribution":
        amt = float(getattr(initial, "amount", 500.0)) if (initial and getattr(initial, "type", None) == "InvestmentContribution") else 500.0
        gk = getattr(initial, "growth_key", None) if (initial and getattr(initial, "type", None) == "InvestmentContribution") else None
        c1, c2 = st.columns(2)
        a = c1.number_input("Contribution per period (positive number, will be emitted as negative cash)", value=amt, step=50.0, key=f"{key_prefix}_inv_amt")
        gkey = c2.text_input("Optional growth multiplier key (leave blank for fixed)", value=gk or "", key=f"{key_prefix}_inv_gk")
        return InvestmentContributionValue(amount=float(a), growth_key=gkey or None)

    if vg_type == "TaxEvent":
        rt = float(getattr(initial, "rate", 0.20)) if (initial and getattr(initial, "type", None) == "TaxEvent") else 0.20
        bk = getattr(initial, "base_key", "taxable_income") if (initial and getattr(initial, "type", None) == "TaxEvent") else "taxable_income"
        tk = getattr(initial, "tax_key", "tax_paid") if (initial and getattr(initial, "type", None) == "TaxEvent") else "tax_paid"
        c1, c2, c3 = st.columns(3)
        rate = c1.number_input("Tax rate (e.g. 0.22)", value=rt, step=0.01, format="%.2f", key=f"{key_prefix}_tax_rate")
    ...
    if vg_type == "RateChange":
        ...
        upk = getattr(initial, "update_key", "some_rate") if (initial and getattr(initial, "type", None) == "RateChange") else "some_rate"
        up_key = st.text_input("State key to update with the sampled value", value=upk, key=f"{key_prefix}_rate_upkey")
